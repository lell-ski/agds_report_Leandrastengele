...
implement GDD Fn (cumulative sum of temperatures above Threshhold T0 = 5°C) --> but not done with an actual Fn in R
```{r}
...
DO I GET AN ISSUE WHEN I USE INDICES OF TEMP BC ITS MORE THAN 1 TEMP PER DAY?
```{r}
gdd_model <- function(temp, par) {
  ...
  # weights
  #weight_factor <- par[3]
  #day_idx <- 1:length(temp)
  #specific_weight <- weight_factor*day_idx
